...
namespace MuseumObserverLogic.Entity
{
    class Restoration
    {
        ...
        public bool Save(DataSetMuseum dataSet, Connection conn, Transaction tr)
        {
            ...
            dataAdapter.InsertCommand = new System.Data.SqlClient.SqlCommand(string.Format("INSERT INTO Restoration (ID, ExhibitID, RestorerID, Start, [End], Photo, Description) " +
                                                                                                        "VALUES (@ID, @ExhibitID, @RestorerID, @Start, @[End], @Photo, @Description);"),
            ...
            dataAdapter.UpdateCommand = new System.Data.SqlClient.SqlCommand(string.Format("UPDATE Restoration SET ExhibitID = @ExhibitID, RestorerID = @RestorerID, Start = @Start, [End] = @[End], Photo = @Photo, Description = @Description WHERE ID = @ID;"), conn.getConnection(), tr.getTransaction());
